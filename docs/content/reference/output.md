...
describes what users get. Pick a format with `-o`, or let baomoi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
baomoi <command> -o table   # aligned columns for reading
baomoi <command> -o jsonl   # one JSON object per line, for piping
baomoi <command> -o json    # a single JSON array
baomoi <command> -o csv     # spreadsheet friendly
baomoi <command> -o tsv     # tab-separated
baomoi <command> -o url     # just the URL column
baomoi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
baomoi <command> --fields id,title,url
```
...
```bash
baomoi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
baomoi <command>            # a table, because this is a terminal
baomoi <command> | wc -l    # JSONL, because this is a pipe
```
